...
void StatusReporter::PrintStatusLine(const RuntimeStatusSnapshot &snapshot)
{
    ...
    printf("\r\033[K");
    printf("Out:%u(%.0f%%%+.0f) GPU:%u/%u Ratio:%.6f(%+.3f%%) Target:%.0f%% %llus",
           snapshot.outputBufferFrames, outputFillPercent, snapshot.outputFillRate, snapshot.busySlots,
           snapshot.totalSlots, snapshot.currentRatio, ratioDeviation, targetFillPercent,
